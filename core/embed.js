...
(function () {
  ...
  iframe.style.cssText = 'width:100%;height:100%;border:0;background:transparent;color-scheme:normal;visibility:hidden;opacity:0;transition:opacity 0.2s ease;';
  ...
  function applyExpandedSize() {
    if (iframe.style.visibility === 'hidden') return;
  ...
  // 5) 展開 / 收合（使用 visibility 而非 display，避免關閉時打斷語音）
  function setOpen(open) {
    if (open) {
      ...
      // 使用 visibility: hidden 而非 display: none，避免 Chrome 停止 speechSynthesis
